...
static int sunxi_lcd_enable(struct udevice *dev, int bpp,
  ...
  lcdc_pll_set(ccm, 0, edid->pixelclock.typ / 1000, &clk_div, &clk_double,
               false);
#else
  struct clk bus_clk, tcon_clk;
  struct reset_ctl rst;

  ret = reset_get_by_name(dev, "bus", &rst);
  if (!ret)
    reset_deassert(&rst);
  ...
  ret = clk_get_by_name(dev, "tcon", &tcon_clk);
  if (!ret) {
    clk_set_rate(&tcon_clk, edid->pixelclock.typ);
    clk_enable(&tcon_clk);
  }

  /* For T113-S, dividers are handled by CCU, assuming 1:1 or logic in clk
   * driver */
  clk_div = 1;
  clk_double = 0;
  ...
  lcdc_tcon0_mode_set(lcdc, edid, clk_div, false, priv->panel_bpp,
...
static int sunxi_lcd_probe(struct udevice *dev) {
  ...
  if (fdtdec_decode_display_timing(gd->fdt_blob, dev_of_offset(cdev), 0,
                                   &priv->timing)) {
    debug("%s: Failed to decode display timing\n", __func__);
    return -EINVAL;
  }
  timing_node =
      fdt_subnode_offset(gd->fdt_blob, dev_of_offset(cdev), "display-timings");
...
#ifdef CONFIG_MACH_SUN50I
U_BOOT_DRVINFO(sunxi_lcd) = {.name = "sunxi_lcd"};
#endif
