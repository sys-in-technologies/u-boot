...
static void sunxi_de2_composer_init(struct udevice *dev) {
#ifndef CONFIG_MACH_SUN8I_R528
  ...
  clrsetbits_le32(&ccm->de_clk_cfg, CCM_DE2_CTRL_PLL_MASK, CCM_DE2_CTRL_PLL10);
  ...
  ret = reset_get_by_name(dev, "de", &rst);
  if (!ret)
    reset_deassert(&rst);
  ...
  if (!ret) {
    clk_set_rate(&mod_clk, 300000000);
    clk_enable(&mod_clk);
  }
#endif
}
...
static void sunxi_de2_mode_set(int mux, const struct display_timing *mode,
  ...
  struct de_glb *const de_glb_regs =
      (struct de_glb *)(de_mux_base + SUNXI_DE2_MUX_GLB_REGS);
  struct de_bld *const de_bld_regs =
      (struct de_bld *)(de_mux_base + SUNXI_DE2_MUX_BLD_REGS);
  struct de_ui *const de_ui_regs =
      (struct de_ui *)(de_mux_base + SUNXI_DE2_MUX_CHAN_REGS +
                       SUNXI_DE2_MUX_CHAN_SZ * 1);
  struct de_csc *const de_csc_regs =
      (struct de_csc *)(de_mux_base + SUNXI_DE2_MUX_DCSC_REGS);
  ...
  for (channel = 0; channel < 4; channel++) {
    ...
    memset(ch, 0, (channel == 0) ? sizeof(struct de_vi) : sizeof(struct de_ui));
  }
  ...
  writel(mode->flags & DISPLAY_FLAGS_INTERLACED ? 2 : 0, &de_bld_regs->out_ctl);
...
static int sunxi_de2_init(struct udevice *dev, ulong fbbase,
                          enum video_log2_bpp l2bpp, struct udevice *disp,
                          int mux, bool is_composite) {
  ...
  efi_add_memory_map(fbbase,
                     timing.hactive.typ * timing.vactive.typ * (1 << l2bpp) / 8,
...
static int sunxi_de2_probe(struct udevice *dev) {
  ...
  ret = uclass_get_device_by_driver(UCLASS_DISPLAY, DM_DRIVER_GET(sunxi_lcd),
                                    &disp);
  if (!ret) {
    int mux;
...
U_BOOT_DRVINFO(sunxi_de2) = {.name = "sunxi_de2"};
...
int sunxi_simplefb_setup(void *blob) {
  ...
  ret =
      uclass_get_device_by_driver(UCLASS_VIDEO, DM_DRIVER_GET(sunxi_de2), &de2);
  if (ret) {
  ...
  ret = fdt_setup_simplefb_node(
      blob, offset, de2_plat->base, de2_priv->xsize, de2_priv->ysize,
      VNBYTES(de2_priv->bpix) * de2_priv->xsize, "x8r8g8b8");
  if (ret)
