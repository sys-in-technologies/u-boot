...
static int lcdc_get_clk_delay(const struct display_timing *mode, int tcon) {
  ...
  delay = mode->vfront_porch.typ + mode->vsync_len.typ + mode->vback_porch.typ;
...
void lcdc_tcon0_mode_set(struct sunxi_lcdc_reg *const lcdc,
                         const struct display_timing *mode, int clk_div,
                         bool for_ext_vga_dac, int depth, int dclk_phase) {
  ...
  writel(SUNXI_LCDC_TCON0_DCLK_ENABLE | SUNXI_LCDC_TCON0_DCLK_DIV(clk_div),
         &lcdc->tcon0_dclk);

  writel(SUNXI_LCDC_X(mode->hactive.typ) | SUNXI_LCDC_Y(mode->vactive.typ),
  ...
  if (depth == 18 || depth == 16) {
    ...
    writel(((depth == 18) ? SUNXI_LCDC_TCON0_FRM_CTRL_RGB666
...
void lcdc_tcon1_mode_set(struct sunxi_lcdc_reg *const lcdc,
                         const struct display_timing *mode, bool ext_hvsync,
                         bool is_composite) {
  ...
  writel(SUNXI_LCDC_TCON1_CTRL_ENABLE |
             ((mode->flags & DISPLAY_FLAGS_INTERLACED)
                  ? SUNXI_LCDC_TCON1_CTRL_INTERLACE_ENABLE
                  : 0) |
             SUNXI_LCDC_TCON1_CTRL_CLK_DELAY(clk_delay),
         &lcdc->tcon1_ctrl);
  ...
  writel(SUNXI_LCDC_TCON1_TIMING_H_TOTAL(total) |
             SUNXI_LCDC_TCON1_TIMING_H_BP(bp),
         &lcdc->tcon1_timing_h);
  ...
  writel(SUNXI_LCDC_TCON1_TIMING_V_TOTAL(total) |
             SUNXI_LCDC_TCON1_TIMING_V_BP(bp),
         &lcdc->tcon1_timing_v);
  ...
  if (ext_hvsync) {
    ...
    clrbits_le32(&lcdc->tcon1_io_tristate,
                 SUNXI_LCDC_TCON_VSYNC_MASK | SUNXI_LCDC_TCON_HSYNC_MASK);
  }
...
void lcdc_pll_set(struct sunxi_ccm_reg *ccm, int tcon, int dotclock,
  ...
  if (use_mipi_pll) {
    ...
    debug("dotclock: %dkHz = %dkHz via mipi pll\n", dotclock,
          clock_get_mipi_pll() / best_m / 1000);
  } else
#endif
  {
    clock_set_pll3(best_n * step * 1000);
    debug("dotclock: %dkHz = %dkHz: (%d * %dkHz * %d) / %d\n", dotclock,
          (best_double + 1) * clock_get_pll3() / best_m / 1000, best_double + 1,
          step, best_n, best_m);
  }
  ...
  if (tcon == 0) {
    ...
    writel(
        CCM_LCD_CH1_CTRL_GATE |
            ...
            CCM_LCD_CH1_CTRL_M(best_m),
        &ccm->lcd0_ch1_clk_cfg);
    if (is_composite)
      setbits_le32(&ccm->lcd0_ch1_clk_cfg, CCM_LCD_CH1_CTRL_HALF_SCLK1);
  }
#endif
