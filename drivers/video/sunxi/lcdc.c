...
void lcdc_enable(struct sunxi_lcdc_reg * const lcdc, int depth)
{
	...
	log_debug("LCDC: TCON enabled. CTRL=0x%08x TCON0_CTRL=0x%08x IO_POL=0x%08x\n",
			 ...
			 int depth, int dclk_phase)
{
	 ...
	 * These are at fixed offsets not covered by the lcdc struct. */
	{
		...
		log_debug("TCON: TRI: block_space=%u start_delay=%u htotal=%u vtotal=%u\n",
