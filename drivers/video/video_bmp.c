                      ...
                      bool align)
{
	...
	if (env_get_yesno("backlight_auto_on") == 1) {
	  /* Enable backlight after successfully drawing a splash image */
                ...
                bl_ret = uclass_get_device(UCLASS_PANEL_BACKLIGHT, 0, &backlight);
                if (!bl_ret) {
                        log_debug("VIDEO: Enabling backlight via uclass...\n");
                        backlight_enable(backlight);
                }
#ifdef CONFIG_ARCH_SUNXI
                else {
                        /*
                         * Manual fallback for Argon board (PD17 is the enable pin).
                         * This matches the logic in sunxi_lcd.c.
                         */
                        log_debug("VIDEO: Enabling sunxi manual backlight (PD17)...\n");
